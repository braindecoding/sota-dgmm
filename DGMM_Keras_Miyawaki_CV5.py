...
# -*- coding: utf-8 -*-
"""
DGMM Keras with Miyawaki Dataset - 5-Fold Cross Validation
Based on DGMM_Keras_Miyawaki.py with CV enhancement for robust evaluation

@author: duchangde (adapted for Miyawaki dataset with CV)
"""
...
# Configuration
N_FOLDS = 5
MODEL_SAVE_PATH = 'dgmm_miyawaki_cv_model_checkpoint.pkl'
KERAS_MODEL_PATH = 'dgmm_miyawaki_cv_keras_model.keras'
...
try:
    ...
    # Extract data using standard structure
    Y_full_train = miyawaki_data['fmriTrn']  # (107, 967)
    X_full_train = miyawaki_data['stimTrn']  # (107, 784)
    Y_test = miyawaki_data['fmriTest']       # (12, 967)
    X_test = miyawaki_data['stimTest']       # (12, 784)
...
# Determine image dimensions (28x28 from 784 pixels)
resolution = 28
...
# Hyper-parameters (SAME AS ORIGINAL DGMM)
tau_alpha = 1
tau_beta = 1
eta_alpha = 1
eta_beta = 1
...
def build_model():
    """Build DGMM model architecture (SAME AS ORIGINAL)"""
    ...
    # Decoder layers (SAME AS ORIGINAL)
...
print(f"\n🔄 Starting {N_FOLDS}-Fold Cross Validation on Miyawaki Dataset...")
print("="*60)
...
for fold_idx, (train_indices, val_indices) in enumerate(kfold.split(X_full_train)):
    ...
    # Initialize parameters for this fold (SAME AS ORIGINAL)
    ...
    print(f"✅ Similarity matrix computed: {S.shape} (NO DATA LEAKAGE!)")
    print(f"📊 Training: {Y_train.shape[0]} samples")
    print(f"📊 Validation: {Y_validation.shape[0]} samples")

    # Training loop for this fold (SAME AS ORIGINAL)
    ...
    # Reconstruction and evaluation for this fold (SAME AS ORIGINAL)
    print(f"🔍 Performing reconstruction for fold {fold_idx + 1}...")

    # Reconstruct test images using SAME algorithm as original
    ...
    for i in range(numTest):
        ...
        # Compute latent variables using SAME formula as original
        ...
        # Handle shape mismatch (SAME as original)
    ...
    plt.suptitle(f'DGMM Miyawaki CV Fold {fold_idx + 1} - Reconstruction Results')
    plt.savefig(f'dgmm_miyawaki_cv_fold_{fold_idx + 1}_reconstruction.png', dpi=150, bbox_inches='tight')
    plt.close()
...
print("\n📊 CROSS VALIDATION RESULTS SUMMARY (MIYAWAKI DATASET):")
print("-" * 50)
...
fold_metrics_df.to_csv('dgmm_miyawaki_cv_fold_metrics.csv', index=False)
print(f"✅ Fold metrics saved: dgmm_miyawaki_cv_fold_metrics.csv")

# Save CV summary
cv_summary_df = pd.DataFrame([cv_summary])
cv_summary_df.to_csv('dgmm_miyawaki_cv_summary.csv', index=False)
print(f"✅ CV summary saved: dgmm_miyawaki_cv_summary.csv")
...
sample_metrics_df.to_csv('dgmm_miyawaki_cv_all_sample_metrics.csv', index=False)
print(f"✅ All sample metrics saved: dgmm_miyawaki_cv_all_sample_metrics.csv")
...
losses_df.to_csv('dgmm_miyawaki_cv_training_losses.csv', index=False)
print(f"✅ Training losses saved: dgmm_miyawaki_cv_training_losses.csv")
...
plt.tight_layout()
plt.savefig('dgmm_miyawaki_cv_summary_plots.png', dpi=150, bbox_inches='tight')
plt.close()
print(f"✅ Summary plots saved: dgmm_miyawaki_cv_summary_plots.png")

print(f"\n🎯 FINAL CV RESULTS (MIYAWAKI DATASET):")
...
print(f"   - dgmm_miyawaki_cv_fold_metrics.csv")
print(f"   - dgmm_miyawaki_cv_summary.csv")
print(f"   - dgmm_miyawaki_cv_all_sample_metrics.csv")
print(f"   - dgmm_miyawaki_cv_training_losses.csv")
print(f"   - dgmm_miyawaki_cv_summary_plots.png")
print(f"   - dgmm_miyawaki_cv_fold_X_reconstruction.png (for each fold)")
print(f"\n🎉 Successfully completed DGMM 5-Fold CV on Miyawaki Dataset!")
print(f"🧠 fMRI → 🖼️ Image Reconstruction with robust statistical evaluation")
